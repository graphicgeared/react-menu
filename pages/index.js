...
const auth_key =
  "BearereyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJERVYifQ.uOFB7h7_Aw6jbA1HSqVJ44tKMO7E1ljz1kV_JddeKL64YCOH57-l1ZX2Lly-Jnhdnxk3xMAeW5FawAgymEaMKA";
  ...
  const getData = (url) => {
    if (!timeoutId) {
      timeoutId = window.setTimeout(function () {
        ...
        fetch(
          "https://uatgecmsus.systemax.com/catalogApis/catalog/feature/" + url,
          {
            method: "GET",
            headers: {
  ...
  const getDatas = (urls) => {
    if (!timeoutIds) {
      timeoutIds = window.setTimeout(function () {
        ...
        fetch(
          "https://uatgecmsus.systemax.com/catalogApis/catalog/feature/" + urls,
          {
            method: "GET",
            headers: {
              Authorization: auth_key,
      ...
      <div className="menu">
        <ul style={{ left: 10, top: 0 }} className="lv1">
          {JsonData.map((info) => (
            <Menu key={info.title}>
              <li>
                {info.childs || info.title == "QUICK ORDER" ? (
                  <a>
                    {info.title} {arrowdown}{" "}
                  </a>
                ) : (
                  <a>{info.title} </a>
                )}
                      ...
                      <ul className="lvn lv2">
                        {info.primaries.map((submenu) => (
                          <Menu key={submenu.description}>
                            <li
                              onMouseOver={() =>
                                getData(submenu.key + "?url=" + submenu.url)
                              }
                              onMouseOut={() => hovoutfirst()}
                            >
                              {submenu.childs ? (
                                <a>
                                  {submenu.description}{" "}
